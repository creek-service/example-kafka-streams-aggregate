...
package org.acme.example.word.counter.kafka.streams;

import static org.acme.example.services.WordCounterServiceDescriptor.WordCountTopic;
import static org.acme.example.services.WordCounterServiceDescriptor.WordTopic;
...
class TopologyBuilderTest {
    ...
    private static final long TWEED_ID = 1L;

    private static CreekContext ctx;

    private TopologyTestDriver testDriver;
    private TestInputTopic<String, Long> words;
    private TestOutputTopic<String, Long> counts;
    private Topology topology;

    @BeforeAll
    public static void classSetup() {
        ctx =
                CreekServices.builder(new WordCounterServiceDescriptor())
    ...
    public void setUp() {
        ...
        words = inputTopic(WordTopic, ctx, testDriver);
        counts = outputTopic(WordCountTopic, ctx, testDriver);
    }
    ...
    @Test
    void shouldOutputWordCounts() {
        // Given:
        words.pipeInput("word", TWEED_ID);
        counts.readKeyValuesToList();

        // When:
        words.pipeInput("word", TWEED_ID);

        // Then:
        assertThat(counts.readKeyValuesToList(), contains(pair("word", 2L)));
    }
    ...
    void shouldNotChangeTheTopologyUnintentionally() {
        ...
        final String expectedTopology =
                TestPaths.readString(
                        TestPaths.moduleRoot("word-counter-service")
